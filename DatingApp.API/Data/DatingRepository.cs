    ...
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            switch(messageParams.MessageContainer){
                case "Inbox":
                    messages = messages.Where(u => u.RecipientId == messageParams.UserId && u.RecipientDeleated ==false);
                    break;
                case "Outbox":
                    messages = messages.Where(u => u.SenderId == messageParams.UserId && u.SenderDeleated == false);
                    break;
                default:
                    messages = messages.Where(u => u.RecipientId == messageParams.UserId && u.RecipientDeleated ==false  && u.IsRead == false);
                    break;
            }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipientId)
        {
            ...
            var messages = await _context.Messages.Include(u => u.Sender).ThenInclude(p => p.Photos)
                           ...
                           .Where(m => m.RecipientId == userId && m.RecipientDeleated == false && m.SenderId == recipientId ||
                                         m.RecipientId == recipientId && m.SenderId== userId && m.SenderDeleated ==false)
